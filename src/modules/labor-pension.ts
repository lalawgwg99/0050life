...
export function projectLaborPension(input: PlanningInput, endMonth: number): LaborPensionProjection {
  ...
  let claimed = false;
  let payoutCount = 0;
  const payoutMonths = Math.max(1, Math.round(input.laborPension.payoutYears * 12));
  ...
  for (let month = asOf; month < endMonth; month += 1) {
    ...
    if (month === claimMonth) {
      ...
      initialMonthlyNominal = actuarialMonthlyRate === 0
        ...
        : (balance * actuarialMonthlyRate) / (1 - Math.pow(1 + actuarialMonthlyRate, -payoutMonths));
    }
    ...
    if (!claimed) {
      ...
      balance *= 1 + actuarialMonthlyRate;
