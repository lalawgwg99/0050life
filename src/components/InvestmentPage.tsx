  ...
  return <main className="investment-page">
    ...
    <div className="investment-layout"><section aria-label="投資條件">
      ...
      {"error" in calculation ? <p role="alert">{calculation.error}</p> : <>
        <div className="investment-answer"><span style={{color: "var(--label-secondary)", fontSize: "14px", fontWeight: 600}}>{years} 年後預估資產 (今日幣值)</span><h2>{formatMoney(calculation.result.final.today)}</h2>
          {calculation.monthly !== null && <><p><strong>達到目標：現在每月共投入約 {formatMoney(calculation.monthly)}</strong></p><button type="button" onClick={() => setHoldings(calculation.suggested!)}>套用每月投入建議</button></>}
        ...
        <section className="investment-handoff"><h2>接著看退休生活夠不夠</h2><p>將投資資料帶回退休規劃，整合勞保與勞退，檢視整體現金流。</p>
