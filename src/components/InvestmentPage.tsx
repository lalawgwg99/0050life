...
import { calculateInvestment, solveMonthlyInvestment } from "../engine/investment-tool";
...
export function InvestmentPage({ input, onImport }: { input: PlanningInput; onImport: (holdings: PlanningInput["investment"]["holdings"], growth: number) => void }) {
  ...
  const calculation = useMemo(() => {
    try {
      ...
      return { result, scenarios: [-0.02, 0, 0.02].map(delta => calculateInvestment(plan, delta)), monthly: mode === "target" ? solveMonthlyInvestment(plan, target) : null };
    ...
    <div className="investment-layout"><section aria-label="投資條件">
      ...
      {"error" in calculation ? <p role="alert">{calculation.error}</p> : <>
        <div className="investment-answer"><span>以今天物價看，{years} 年後預計剩下</span><h2>{formatMoney(calculation.result.final.today)}</h2><p>依你填的報酬試算，不是保證收益。</p>
          {calculation.monthly !== null && <p><strong>達到目標：現在每月共投入約 {formatMoney(calculation.monthly)}</strong><br />按目前各筆投入比例分配；都為 0 時放在第一筆，並沿用每年投入調整。下方仍顯示目前輸入的結果。</p>}
        ...
        <section className="investment-handoff"><h2>接著看退休生活夠不夠</h2><p>只帶回目前本金、每月投入、報酬、費用與投入調整，不帶回期末金額，避免重複複利。試算年數、領取、物價與單筆加碼不覆蓋退休設定；勞退請在退休頁設定，避免重複加入。</p><button type="button" onClick={() => setConfirm(true)}>帶入退休規劃</button>{confirm && <div><p>將取代退休頁原有的投資明細，其他退休條件不變。是否繼續？</p><button type="button" onClick={() => onImport(holdings, growth)}>確認取代投資資料</button><button type="button" onClick={() => setConfirm(false)}>取消</button></div>}</section>
      </>}
